...
class ProductTemplate(models.Model):
    ...
    @api.onchange('dollar_price')
    def onchange_dollar_price(self):
        rate= self.env['res.currency.line'].get_last_rate()
        self.list_price= self.dollar_price * rate
    ...
    def action_view_dollar_rate(self):
        self.ensure_one()
        action = self.env.ref('dollar_rate.action_currency_rate').read()[0]
        # action['domain'] = [
